...
bool 			operator != (const node &v){return this->p != v.p;}
    ...
    node *myFind(key_type index,node *root,node**p)  const
    {
        ...
        {   
            // std::cout << "left:"<<root->p->first << std::endl;
            // *p = new value_type(root->p->first,root->p->second);
            *p = new node(*root);
    ...
    node *myFind1(key_type index,node *root,node**p)  const
    {
        if (!root)
            return root;
        if (_comp(index, root->p->first))
        {   
            ...
            return myFind1(index,root->left,p);
        }
        else if (_comp(root->p->first,index))
        {   
            std::cout << "right:"<<root->p->first << std::endl;
            *p = new node(*root);
            ...
            return myFind1(index,root->right,p);
        }
        else 
            return root;
    }
    // node *myFind(key_type index,node *root,bool &found,value_type*p) const
    // {
    //         std::cout << "Myfind" << std::endl;
    ...
    //         p = new value_type(root->p->first,root->p->second);
    //         return myFind(index,root->right,found,p);
    //     }
    //     else if (!_comp(index, root->p->first) && !_comp(index, root->p->first) )
    //     {
